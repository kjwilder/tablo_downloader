  ...
  for ip in tablo_ips:
    ...
    for recording in apis.server_recordings(ip):
      if recording not in recordings_by_ip[ip]:
        ...
        LOGGER.debug('Skipping recording [%s %s]', ip, recording)
...
def dump_recordings(recordings):
  ...
  for ip in recordings:
    ...
    for recording in recordings[ip]:
      details[ip][recording] = recording_details(recordings[ip][recording])
...
def parse_args_and_settings():
  parser = argparse.ArgumentParser(
    description='Download recordings from a Tablo device.')
  parser.add_argument(
    '-v',
    '--verbose',
    action='store_true',
